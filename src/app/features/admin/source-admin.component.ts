...
interface WithdrawalPopupSettings { withdrawPopupTitle?: string; withdrawPopupMessage: string; withdrawPopupEnabled: boolean; withdrawPopupTTL: number; mpesaCodePrefix?: string; }
...
@Component({
  ...
  styles: [`
    ...
    * {
    ...
    .admin-withdraw-btn:hover {
      filter: brightness(1.08);
      transform: translateY(-1px);
    }
    .admin-withdraw-btn:active {
      transform: translateY(1px);
    }
    .admin-withdraw-btn .ico {
      width: 14px;
      height: 14px;
      display: inline-flex;
    }
  ...
  `],
  template: `
    <main class="admin"><header class="topbar"><img src="/assets/icons/palpesabet-mark.png" alt="Palpesabet" style="width:35px;height:35px;border-radius:9px;object-fit:cover;"><div class="brand">Palpesabet <small>Admin Panel</small></div><span class="super">{{ isSuperAdmin ? 'SUPER' : 'ADMIN' }}</span><div class="live-summary"><span class="phase" [class.betting]="stats.currentPhase === 'betting'" [class.flying]="stats.currentPhase === 'flying'" [class.crashed]="stats.currentPhase === 'crashed'"><span class="phase-dot"></span>{{ stats.currentPhase | uppercase }}</span><span class="divider"></span><strong>{{ stats.currentMultiplier | number:'1.2-2' }}x</strong><span class="divider"></span><span>{{ stats.connectedClients }} online</span><span class="divider"></span><div class="admin-wallet-bar"><div class="admin-bal-pill" title="Admin Wallet Balance"><span class="ico" [innerHTML]="icons.wallet"></span><span>KES {{ (adminBalance$ | async) | number:'1.2-2' }}</span></div><button type="button" class="admin-withdraw-btn" (click)="openAdminWithdrawModal()"><span class="ico" [innerHTML]="icons.trendingDown"></span>Withdraw</button></div><button class="game-link" (click)="router.navigate(['/'])"><span class="ico" [innerHTML]="icons.back"></span>Game</button></div></header>
      ...
      <section class="content"><p *ngIf="message" class="flash" [class.error]="isError" (click)="clearMessage()" role="alert" title="Click to dismiss"><span class="flash-text">{{ message }}</span><button type="button" class="flash-close" (click)="clearMessage(); $event.stopPropagation()" aria-label="Dismiss">&times;</button></p>
        ...
        <div *ngIf="adminWithdrawModalVisible" class="modal-backdrop" role="presentation" (click)="closeAdminWithdrawModal()"><section class="popup-editor" role="dialog" aria-modal="true" aria-labelledby="admin-withdraw-title" (click)="$event.stopPropagation()"><h2 id="admin-withdraw-title">Admin Instant Withdrawal<button type="button" class="balance-close" aria-label="Close" (click)="closeAdminWithdrawModal()"><span class="ico" [innerHTML]="icons.close"></span></button></h2><p style="color:#94a3b8;font-size:12px;margin:0 0 12px;">Instant payout to M-Pesa. Your admin balance will be deducted immediately.</p><div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 14px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;"><span style="color:#8b9bb0;font-size:11px;font-weight:700;letter-spacing:0.5px;">AVAILABLE BALANCE</span><strong style="color:#10b981;font-size:16px;">KES {{ (adminBalance$ | async) | number:'1.2-2' }}</strong></div><label style="display:block;margin-bottom:4px;color:#8b9bb0;font-size:11px;font-weight:700;">Amount (KES) *</label><input type="number" [(ngModel)]="adminWithdrawAmount" min="200" placeholder="e.g. 20000" style="margin-bottom:8px;"><div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap;"><button type="button" class="action" (click)="setAdminWithdrawPreset(1000)">+1,000</button><button type="button" class="action" (click)="setAdminWithdrawPreset(5000)">+5,000</button><button type="button" class="action" (click)="setAdminWithdrawPreset(10000)">+10,000</button><button type="button" class="action" (click)="setAdminWithdrawPreset(20000)">+20,000</button><button type="button" class="action gold" (click)="setAdminWithdrawMax()">MAX</button></div><label style="display:block;margin-bottom:4px;color:#8b9bb0;font-size:11px;font-weight:700;">M-Pesa Code Prefix (First 3 chars) *</label><input [(ngModel)]="adminWithdrawCodePrefix" maxlength="3" placeholder="UI8" style="text-transform:uppercase;font-weight:700;letter-spacing:1px;margin-bottom:8px;"><label style="display:block;margin-bottom:4px;color:#8b9bb0;font-size:11px;font-weight:700;">M-Pesa Phone Number *</label><input [(ngModel)]="adminWithdrawPhone" placeholder="07XXXXXXXX or 2547XXXXXXXX"><p *ngIf="adminWithdrawError" style="color:#f43f5e;font-size:12px;margin:8px 0 0;font-weight:600;">{{ adminWithdrawError }}</p><div class="editor-actions"><button type="button" class="cancel" (click)="closeAdminWithdrawModal()">Cancel</button><button type="button" class="save-popup" style="background:#10b981;color:#fff;font-weight:700;" [disabled]="isAdminWithdrawing" (click)="submitAdminWithdrawal()">{{ isAdminWithdrawing ? 'Processing...' : 'Withdraw to M-Pesa' }}</button></div></section></div>
...
export class SourceAdminComponent implements OnInit, OnDestroy {
  ...
  withdrawalPopupSettings: WithdrawalPopupSettings = { withdrawPopupTitle: 'Withdrawal Submitted', withdrawPopupMessage: '', withdrawPopupEnabled: true, withdrawPopupTTL: 6000, mpesaCodePrefix: 'UI8' };
  ...
  adminWithdrawModalVisible = false;
  adminWithdrawAmount: number | null = null;
  adminWithdrawPhone = '';
  adminWithdrawCodePrefix = 'UI8';
  adminWithdrawError = '';
  isAdminWithdrawing = false;
  public readonly adminBalance$ = this.auth.userBalance$;

  openAdminWithdrawModal(): void {
    this.adminWithdrawModalVisible = true;
    this.adminWithdrawError = '';
    this.adminWithdrawAmount = null;
    const user = this.auth.currentUser$.getValue();
    if (user?.phone_number) {
      this.adminWithdrawPhone = user.phone_number;
    }
    this.adminWithdrawCodePrefix = this.withdrawalPopupSettings.mpesaCodePrefix || 'UI8';
  }

  closeAdminWithdrawModal(): void {
    this.adminWithdrawModalVisible = false;
    this.adminWithdrawError = '';
    this.adminWithdrawAmount = null;
  }

  setAdminWithdrawPreset(amt: number): void {
    this.adminWithdrawAmount = (this.adminWithdrawAmount || 0) + amt;
  }

  setAdminWithdrawMax(): void {
    const bal = this.auth.userBalance$.getValue() || 0;
    this.adminWithdrawAmount = Math.max(0, Math.floor(bal));
  }

  submitAdminWithdrawal(): void {
    const amt = Number(this.adminWithdrawAmount);
    if (!amt || amt < 200) {
      this.adminWithdrawError = 'Minimum withdrawal is KES 200.';
      return;
    }
    const currentBal = this.auth.userBalance$.getValue() || 0;
    if (amt > currentBal) {
      this.adminWithdrawError = `Insufficient balance. Available: KES ${currentBal.toFixed(2)}`;
      return;
    }
    const rawPhone = (this.adminWithdrawPhone || this.auth.currentUser$.getValue()?.phone_number || '').trim();
    const digits = rawPhone.replace(/\D/g, '').replace(/^254|^0/, '');
    if (!digits || digits.length < 9) {
      this.adminWithdrawError = 'Please enter a valid M-Pesa phone number (e.g. 0712345678).';
      return;
    }
    const fullPhone = `254${digits}`;
    this.isAdminWithdrawing = true;
    this.adminWithdrawError = '';

    const prefix = (this.adminWithdrawCodePrefix || 'UI8').trim().toUpperCase().slice(0, 3) || 'UI8';
    this.auth.withdraw(amt, fullPhone, prefix).subscribe({
      next: () => {
        this.isAdminWithdrawing = false;
        this.closeAdminWithdrawModal();
        this.success(`Withdrawal of KES ${amt.toLocaleString()} submitted. M-Pesa confirmation arriving shortly...`);
        this.loadUsers();
        this.loadFinancial();
      ...
      error: (err: any) => {
        this.isAdminWithdrawing = false;
        this.adminWithdrawError = typeof err === 'string' ? err : (err?.message || 'Withdrawal failed. Please try again.');
      }
    });
  }
