...
@Component({
  ...
  `],
  template: `
    <main class="admin"><header class="topbar"><img src="/assets/icons/pakabet-mark.png" alt="Pakabet" style="width:35px;height:35px;border-radius:9px;object-fit:cover;"><div class="brand">Pakabet <small>Admin Panel</small></div><span class="super">{{ isSuperAdmin ? 'SUPER' : 'ADMIN' }}</span><div class="live-summary"><span class="phase" [class.betting]="stats.currentPhase === 'betting'" [class.flying]="stats.currentPhase === 'flying'" [class.crashed]="stats.currentPhase === 'crashed'"><span class="phase-dot"></span>{{ stats.currentPhase | uppercase }}</span><span class="divider"></span><strong>{{ stats.currentMultiplier | number:'1.2-2' }}x</strong><span class="divider"></span><span>{{ stats.connectedClients }} online</span><button class="game-link" (click)="router.navigate(['/'])"><span class="ico" [innerHTML]="icons.back"></span>Game</button></div></header>
